...
That inference is testable, and it is tested surprisingly rarely. Surveying recent QNLP papers
that claim a quantum benefit on text, we find that the large majority compare against classical
\emph{networks} of matched size but not against a classical \emph{simulation of their own
circuit}---the one baseline that would isolate whether the quantum resource, rather than the
...
not merely a separately-trained classical baseline. The exact search and inclusion criteria are
in Appendix~\ref{app:survey}.} The same gap was recently closed for tabular
...
\appendix
\section{Literature-survey methodology}\label{app:survey}
\emph{To be completed:} search terms, date cutoff, inclusion/exclusion counts, and the
selection-bias check for the motivating survey in Section~1.
\section{Model circuits and glossary}\label{app:circuits}
\emph{To be completed:} per-model ansätze (gate glyphs, repeated-block notation), the
mixed-state purification path for \texttt{qmsan}, and a glossary keying physics terms for NLP
readers.
\section{Datasets and preprocessing}\label{app:data}
\emph{To be completed:} sizes, splits, token-embedding construction, and the exact released
indices.
\section{Configurations and compute}\label{app:configs}
\emph{To be completed:} hyperparameters per model, seeds, wall-clock/RAM, and the tolerance
robustness study for \chistar.
\end{document}
